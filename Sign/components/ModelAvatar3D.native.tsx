...
import { Canvas, useFrame, useGraph } from '@react-three/fiber/native';
...
function AvatarModel({ signData, isPlaying, speed, onSignComplete, onReady, modelUri }: Props & { modelUri: string }) {
  ...
  useFrame((state, delta) => {
    ...
    // Skip frames with empty body data
...
// Pre-resolve the .glb asset URI for native platforms
const modelAsset = Asset.fromModule(require('../assets/models/female.glb'));
...
export default function ModelAvatar3D(props: Props) {
  ...
  useEffect(() => {
    ...
    console.log('[ModelAvatar3D] asset.uri:', asset.uri, 'localUri:', asset.localUri);

    // On native, need to download first to get a localUri
    if (typeof asset.downloadAsync === 'function') {
      asset.downloadAsync().then(() => {
        const uri = typeof asset.uri === 'string' ? asset.uri : (asset.localUri || '');
        console.log('[ModelAvatar3D] downloaded uri:', uri);
        setModelUri(uri);
        props.onReady?.();
      }).catch((err) => {
        console.error('[ModelAvatar3D] download failed:', err);
      });
      return;
    }

    // Fallback: use asset.uri if it's already a string
    const directUri = typeof asset.uri === 'string' ? asset.uri : undefined;
    if (directUri) {
    ...
    // Last resort: raw require
    try {
      const rawAsset: any = require('../assets/models/female.glb');
      const fallback = typeof rawAsset === 'string'
        ? rawAsset
        : (rawAsset?.uri || rawAsset?.default?.uri || '');
      console.log('[ModelAvatar3D] fallback uri:', fallback);
      if (fallback && typeof fallback === 'string') {
        setModelUri(fallback);
        props.onReady?.();
      }
    } catch (err) {
      console.error('[ModelAvatar3D] require fallback failed:', err);
    }
  }, []);
