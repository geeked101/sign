...
function ChibiPandaAvatar({ signData, isPlaying, speed, onSignComplete }: Props) {
  ...
  useFrame((state, delta) => {
    ...
    if (isPlaying && (!signData || !signData.frames || signData.frames.length === 0)) {
      ...
      // Track how long we've been shrugging
      progressRef.current += delta;
      if (progressRef.current >= 1.5) { // Shrug for 1.5 seconds
      ...
      // Move hands into a 'searching' pose
    ...
    if (isTransitioningRef.current) {
      ...
      if (currentTotalIndex >= framesCount - 1) {
        // Sign Finished
        lastPoseRef.current = signData.frames[framesCount - 1]; // Save final pose
        if (onSignComplete) onSignComplete();
        progressRef.current = 0;
      }
  ...
  return (
    <group ref={groupRef}>
      ...
      {BODY_CONNECTIONS.map((_, i) => (<Line key={`b-${i}`} ref={el => { bodyLinesRefs.current[i] = el; }} points={[[0,0,0], [0,0,0]]} color={WHITE} lineWidth={10} />))}
      {HAND_CONNECTIONS.map((_, i) => (
        <group key={`h-grp-${i}`}>
          <Line ref={el => { leftHandLinesRefs.current[i] = el; }} points={[[0,0,0], [0,0,0]]} color={NEON} lineWidth={3} />
          <Line ref={el => { rightHandLinesRefs.current[i] = el; }} points={[[0,0,0], [0,0,0]]} color={NEON} lineWidth={3} />
        </group>
      ))}
    </group>
  );
